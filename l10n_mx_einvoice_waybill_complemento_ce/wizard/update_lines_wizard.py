...
class ImportLinesDetailWaybill(models.TransientModel):
        ...
        for invoice in invoices:
            ...
            for line in invoice.invoice_line_ids:
                merchandise_vals = {
                                        ...
                                        'sat_product_id': line.product_id.sat_product_id.id if line.product_id.sat_product_id else False,
                                        'quantity': line.quantity,
                                        'sat_uom_id': line.product_uom_id.sat_uom_id.id if line.product_uom_id.sat_uom_id else False,
                                        ...
                                        'arancel_id':  line.product_id.sat_arancel_id.id if line.product_id.sat_arancel_id else False,
    ...
    def create_stcc_info(self,values):
        ...
        if product_record:
            if not sat_product_id:
                sat_product_id = product_record.sat_product_id.id if product_record.sat_product_id else False
            if not sat_uom_id:
                sat_uom_id = product_record.uom_id.sat_uom_id.id if product_record.uom_id.sat_uom_id else False
        ...
        arancel_id = False
        if product_record:
            if product_record.sat_arancel_id:
                arancel_id = product_record.sat_arancel_id.id
            else:
                if fraccion_arancelaria:
                    arancel_id = self.find_arancel(fraccion_arancelaria)
        #### Embalaje ####
    ...
    ### Nuevos Datos ####
    def find_arancel(self, code_arancel):
        arancel_id = False
        arancel_obj = self.env['sat.arancel']
        arancel_search = arancel_obj.search([('code','ilike',code_arancel)], limit=1)
        if arancel_search:
            arancel_id = arancel_search.id
        return arancel_id
    ######## FIN #########
    ...
    def find_sat_product_record(self, code):
        code=code.replace(' ','').replace('\n','')
        sat_product_obj = self.env['sat.producto']
        #Odoo10 : Bussines Process into API Odoo v10 
