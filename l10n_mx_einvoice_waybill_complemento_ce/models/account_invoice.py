...
class InvoiceWaybillTrailerInfo(models.Model):
    ...
    # subtype_rem02_id = fields.Many2one('waybill.tipo.remolque', 'Subtipo Remolque',
    #                                           help="Atributo: SubTipoRem")

    # dolly02_plate_cp = fields.Char('Placa',
    #                                           help="Atributo: Placa")
...
class AccountInvoice(models.Model):
    ...
    cfdi_complemento = fields.Selection(selection_add=[('carta_porte', 'Carta porte')],
        string='Complemento CFDI', ondelete={'carta_porte': 'set default'}, readonly=True, store=True, 
        states={'draft': [('readonly', False)]}, copy=False, default='na', required=True )
    ...
    travel_total_distance_type = fields.Selection([('KM','Kilometros'),('M','Metros')], 'Tipo Distancia Recorrida',
                                                  default="KM")
    ...
    # subtype_rem01_id = fields.Many2one('waybill.tipo.remolque', 'Subtipo Remolque',
    #                                           help="Atributo: SubTipoRem")

    # dolly01_plate_cp = fields.Char('Placa',
    #                                           help="Atributo: Placa")
    ...
    def refresh_complement_waybill_data(self):
        # complement_line_obj = self.env['invoice.line.complement.cp']
        # for rec in self:
        #     if rec.invoice_line_ids:
        #         print ("## UPDATE >>>>>>>>>> ")
        #         for line in rec.invoice_line_ids:
        #             complement_line = complement_line_obj.search([('invoice_line_id','=',line.id)])
        #             if not complement_line:
        #                 vals = {
        #                             'invoice_line_id': line.id,
        #                             'invoice_id': rec.id,
        #                 }
        #                 complement_line_id = complement_line_obj.create(vals)
        return True
...
class InvoiceLineComplementCP(models.Model):
    ...
    sat_product_id = fields.Many2one('sat.producto','Producto SAT')

    quantity = fields.Float('Cantidad', digits=(14,4))

    sat_uom_id = fields.Many2one('sat.udm','UdM SAT')
    ...
    arancel_id  = fields.Many2one('sat.arancel', 'Arancel')

    fraccion_arancelaria = fields.Char('Fraccion Arancelaria', related="arancel_id.code")
    ...
    def onchange_product_id(self):
        if self.product_id:
            ...
            if self.product_id.sat_product_id:
                self.sat_product_id = self.product_id.sat_product_id.id
            if self.product_id.clave_stcc_id:
                if self.tipo_transporte_code == '04':
                    self.clave_stcc_id = self.product_id.clave_stcc_id.id
            if self.product_id.uom_id and self.product_id.uom_id.sat_uom_id:
                self.sat_uom_id = self.product_id.uom_id.sat_uom_id.id
            if self.product_id.tipo_embalaje_id:
                self.tipo_embalaje_id = self.product_id.tipo_embalaje_id.id
            if self.product_id.sat_arancel_id:
                self.arancel_id = self.product_id.arancel_id.id
